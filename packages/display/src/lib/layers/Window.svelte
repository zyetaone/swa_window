<script lang="ts">
	 ...
	 * Composes: Cesium (terrain + clouds post-process) + CSS effect overlays.
	 *
	 * Z-order:
	 *   0: Cesium (terrain, buildings, night lights, clouds, color grading)
	...
	let blindDragY = $state(0); // percentage translateY (0 = closed, -105 = open)
	let dragStartBlindY = 0; // blind position when drag started
	let dragStartPointerY = 0; // pointer Y when drag started
	let blindContainerHeight = 0; // measured height of blind-clip for px-to-% conversion
	const SNAP_THRESHOLD = 0.3; // 30% of travel = snap to opposite state

	// Sync blindDragY with model.blindOpen when not dragging (e.g. programmatic open/close)
	$effect(() => {
		if (!isDragging) {
	...
	function startBlindDrag(pointerY: number) {
		// Don't allow drag during flight transitions
		if (model.isTransitioning) return;
	...
	function moveBlindDrag(pointerY: number) {
		...
		// Clamp between -105 (open) and 0 (closed)
	...
	function endBlindDrag() {
		...
		// Determine snap direction based on how far the blind has traveled
		// from its starting position relative to total travel distance
		const totalTravel = 105; // -105 to 0
		...
		if (travelRatio > SNAP_THRESHOLD) {
			// Traveled far enough — snap to opposite state
			if (dragStartBlindY === 0 || dragStartBlindY > -52.5) {
				// Was closed or near-closed, dragged up enough → open
				model.blindOpen = blindDragY < dragStartBlindY;
			} else {
				// Was open or near-open, dragged down enough → close
				model.blindOpen = blindDragY < dragStartBlindY;
			}
		}
		// If not past threshold, snap back to original state (model.blindOpen unchanged)
		// The $effect will sync blindDragY back to the correct position
	}

	// Clean up any pending long-press timer on component destroy
	onDestroy(() => {
		if (pressTimer) {
	...
	function handleMove(y: number) {
		if (isDragging) {
			moveBlindDrag(y);
			// Cancel long-press if we're actively dragging
			if (pressTimer) {
				clearTimeout(pressTimer);
				pressTimer = null;
			}
	...
	function handleEnd(y: number, x: number) {
		...
		if (isDragging) {
			endBlindDrag();
			return; // Don't process as swipe if we were dragging
		}
	...
	function onTouchStart(e: TouchEvent) {
		...
		handleStart(y, x);
		// Start drag if touching the blind area (lower portion when closed, or handle area)
		startBlindDrag(y);
	}
...
>
	...
	<div class="blind-clip">
		<button
			class="blind-overlay"
			class:discoverable={!model.blindOpen}
...
<style>
	...
	.blind-overlay {
		...
		padding: 0;
		pointer-events: auto;
		/* transform and transition are set via inline style for drag support */
	}
	...
	.blind-overlay.discoverable::after {
		animation: handle-breathe 1.5s ease-in-out infinite;
	}
