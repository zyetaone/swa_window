...
export const COLOR_GRADING_GLSL = `
	...
	void main() {
		...
		// Spatial noise: pseudo-random per-pixel hue variation breaks uniform tinting.
		// Prime coefficients avoid Moire patterns; output range [-0.12, +0.12] shifts
		// the luminance thresholds so neighboring pixels pick different palette colors.
		vec2 uv = v_textureCoordinates;
		float spatialNoise = sin(uv.x * 347.0 + uv.y * 231.0) * 0.06
			+ sin(uv.x * 571.0 - uv.y * 419.0) * 0.04
			+ sin(uv.y * 683.0 + uv.x * 157.0) * 0.02;
		float shiftedLum = lum + spatialNoise;

		// Luminance-based palette: dim outskirts = sodium, mid = amber/warm,
		// bright centers = cool white, brightest = blue-white LED
		vec3 lightColor = mix(sodium, amber, smoothstep(0.15, 0.4, shiftedLum));
		lightColor = mix(lightColor, warmWht, smoothstep(0.4, 0.65, shiftedLum));
		lightColor = mix(lightColor, coolWht, smoothstep(0.65, 0.85, shiftedLum));
		lightColor = mix(lightColor, blueWht, smoothstep(0.85, 1.0, shiftedLum));

		// Per-pixel color temperature jitter: slight warm/cool shift
		float tempJitter = sin(uv.x * 89.0 + uv.y * 53.0) * 0.08;
		lightColor.r += tempJitter * 0.5;
		lightColor.b -= tempJitter * 0.5;
