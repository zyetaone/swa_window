...
import { clamp, lerp, normalizeHeading, AIRCRAFT, FLIGHT_FEEL, AMBIENT, MICRO_EVENTS, WEATHER_EFFECTS, LOCATIONS, LOCATION_MAP } from '@zyeta/shared';
import type { SkyState, LocationId, WeatherType, DisplayMode } from '@zyeta/shared';
...
export class WindowModel {
	...
	cruiseOriginId = $state<LocationId | null>(null);
	...
	cruiseOriginName = $derived(
		this.cruiseOriginId ? (LOCATION_MAP.get(this.cruiseOriginId)?.name ?? this.cruiseOriginId) : null
	);
	...
	// --- Elapsed time for shader uniforms (wraps at 100000s for float32 precision) ---
	elapsedTime = $state(0);
	 ...
	 * Semantic alias: if timezone-aware display is added later, only this getter changes.
	 */
	...
	effectiveCloudDensity = $derived.by(() => {
		...
		// Smoothly reduce cloud opacity as night deepens so Cesium city lights show through.
		// Uses continuous nightFactor (0→1) instead of discrete skyState to prevent pops.
		const nightReduction = lerp(1.0, 0.5, this.nightFactor);
		density *= nightReduction;
		// Enforce night floor for weather types that need minimum cloud cover
		density = Math.max(density, fx.nightCloudFloor * this.nightFactor);
		return density;
	});
	...
	flyTo(locationId: LocationId): void {
		...
		this.cruiseOriginId = this.location; // Capture origin before it's overwritten
	...
	tick(delta: number): void {
		...
		// Wrap at 360000s (~100h) — large enough that sin() discontinuity from the
		// reset is imperceptible, small enough to avoid float64 precision issues.
		this.time = (this.time + delta) % 360000;
		this.elapsedTime = (this.elapsedTime + delta) % 100000;
	...
	private tickDeparture(delta: number): void {
		...
		// After warp peaks (2.5s — matches warpDuration), close blind and transition
	...
	private tickOrbit(delta: number): void {
		...
		const rawAngularSpeed = (AIRCRAFT.DRIFT_RATE * this.flightSpeed) / Math.max(localSpeed, 0.001);
		// Floor prevents stalling on long straights where tangent magnitude is large
		const angularSpeed = Math.max(rawAngularSpeed, AIRCRAFT.MIN_ANGULAR_VELOCITY * this.flightSpeed);
	...
	private tickMotion(delta: number): void {
		...
		const targetBank = clamp(turnRate * FLIGHT_FEEL.BANK_FACTOR, -FLIGHT_FEEL.BANK_ANGLE_MAX, FLIGHT_FEEL.BANK_ANGLE_MAX);
