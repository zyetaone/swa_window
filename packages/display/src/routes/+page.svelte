...
{#if model.isVideoMode && model.videoUrl}
	<!-- Video mode: full-screen video, no flight sim -->
	<main class="app video-mode">
		<video
			src={model.videoUrl}
			autoplay
			loop
...
{:else}
	<main class="app" class:screensaver-mode={model.isScreensaverMode}>
		...
		<!-- Controls (HUD + blind info) — hidden in screensaver mode -->
