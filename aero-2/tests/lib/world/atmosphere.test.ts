...
import { ATMOSPHERE_BANDS, TRANSITION_HALF_WIDTH_M } from '#content/atmosphere/bands.js';
...
describe('resolveAtmosphere', () => {
	...
	it('is continuous — no visible pop anywhere in the climb', () => {
		// A discontinuity here is a seam the audience sees, and on three panes
		// it is a seam that appears on one pane before the others.
		const STEP = 5;
	...
	it('gives the same state whether the altitude is reached climbing or descending', () => {
		// The model is altitude-addressed, not path-addressed. If this ever
		// failed, two panes that arrived at the same altitude from opposite
	...
	it('fills the frame with deck at cruise instead of leaving a void', () => {
		// The whole point: at 35k ft the old model faded ground to nothing and
		// left a black frame. Here the deck below is nearly opaque, so there is
		// something to look at.
		const cruise = resolveAtmosphere(11_600); // ~38,000 ft
	...
	it('leaves every band a core, however the heights get retuned', () => {
		// Regression: a fixed 600 m half-width swallowed the 1 km ground band
		// whole, so no altitude actually meant "ground" — and neighbouring
		// transitions overlapped. Blend width is now derived from the thinner
		// adjacent band. Retuning topM values must not bring that back.
