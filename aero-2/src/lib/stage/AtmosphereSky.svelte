<script lang="ts">
	...
	const groundBlend = $derived(Math.min(0.9, activeAtmosphere.fogDensity * 2200));
</script>

<Sky
	sky-color={rgb(activeAtmosphere.skyTop)}
	horizon-color={rgb(activeAtmosphere.skyHorizon)}
	fog-color={rgb(activeAtmosphere.skyHorizon)}
