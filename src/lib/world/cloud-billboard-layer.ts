...
export class CloudBillboardLayer {
	...
	update(
		...
		for (let i = 0; i < horizonCount; i++) {
			const dLat = rand(-CLOUD_RADIUS_DEG, CLOUD_RADIUS_DEG);
			const dLon = rand(-CLOUD_RADIUS_DEG, CLOUD_RADIUS_DEG);
			const scale = rand(composition.horizon.scaleRange[0], composition.horizon.scaleRange[1]);
			const opacity = rand(0.55, 0.92);
			this.collection.add({
				position: C.Cartesian3.fromDegrees(lon + dLon, lat + dLat, CLOUD_ALT_M),
				image: pickRand(textures),
		...
		for (let i = 0; i < midCount; i++) {
			const dLat = rand(-midRadius, midRadius);
			const dLon = rand(-midRadius, midRadius);
			const scale = rand(composition.mid.scaleRange[0], composition.mid.scaleRange[1]);
			const opacity = rand(0.65, 0.95);
			this.collection.add({
