<script lang="ts">
	...
	import { computeSunDirection, sunElevationSin, moonPhaseFraction, SUN_PLACEMENT_M } from '$lib/world/sky';
	...
	// Angular radius = atan(MOON_RADIUS_M / MOON_PLACEMENT_M). At 6e7 m
	// placement, 2.0e6 gave 1.91° — a 3.8° disc, roughly SEVEN times the real
	// moon's 0.52°, which is what made it dominate the frame rather than sit
	// in it. 1.05e6 halves that to a ~2.0° disc (still ~3.8× life-size, so the
	// procedural cratering and phase are legible at kiosk viewing distance).
	// MOON_HALO_SIZE_M is a multiple of this, so the glow scales with it.
	const MOON_RADIUS_M = 1.05e6;
	...
	/**
	 * Draw order for the disc; the halo takes this minus one.
	 *
	 * Negative because the moon is the FARTHEST thing in the scene and should
	 * sort first. It previously sat at 0/1 while Clouds use the default 0, so
	 * the disc was forced to paint AFTER the clouds — a 384,000 km object
	 * drawing on top of vapour a few km away. Depth testing can't catch it:
	...
	const FS = /* glsl */ `
		...
		void main() {
			...
			// Soft silhouette. The disc is a sphere MESH, so its outline is a
			// hard geometric edge — pin-sharp against the sky in a frame where
			// every other element (clouds, bloom, grain) is soft, which is what
			// made it read as a pasted-on sprite. Feathering alpha across the
			// last few degrees of limb costs nothing and lets the bloom halo
			// carry the edge instead of the polygon boundary.
			float edgeSoft = smoothstep(0.0, 0.16, view);
			gl_FragColor = vec4(color, uVisibility * edgeSoft);
		}
	`;
...
<T.Sprite
	...
	renderOrder={MOON_RENDER_ORDER - 1}
>
