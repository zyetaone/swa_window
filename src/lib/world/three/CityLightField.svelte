		...
		untrack(() => {
			...
			for (let gy = 0; gy < GRID; gy++) {
				for (let gx = 0; gx < GRID; gx++) {
					...
					// Brightness-gated keep-probability: dark suburbs ~8%, VIIRS cores ~100%.
					const keepP = KEEP_BASELINE + (1 - KEEP_BASELINE) * smoothstep((b - BRIGHT_FLOOR) / (BRIGHT_FULL - BRIGHT_FLOOR));
