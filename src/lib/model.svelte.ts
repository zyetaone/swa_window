...
import type { SkyState, LocationId, WeatherType, Location } from '$lib/shared/types';
...
export class WindowModel {
	...
	nightLightScale = $derived(this.nightFactor * this.nightLightIntensity);
	currentLocation: Location = $derived(LOCATION_MAP.get(this.location) ?? LOCATIONS[0]);

	localTimeOfDay = $derived.by(() => {
		const loc = LOCATION_MAP.get(this.location);
		if (!loc) return this.timeOfDay;
		return (this.timeOfDay + loc.utcOffset + 24) % 24;
	});

	nightAltitudeTarget = $derived.by(() => {
		const loc = LOCATION_MAP.get(this.location) ?? LOCATIONS[0];
		if (this.skyState === 'night') return loc.nightAltitude;
		if (this.skyState === 'dusk' || this.skyState === 'dawn') return Math.round((loc.defaultAltitude + loc.nightAltitude) / 2);
