...
/** Shared stream → Uint8Array with byte cap. */
	...
	try {
		while (true) {
			...
			if (received > maxBytes) {
				error(413, `request body too large: received >${received} bytes, limit is ${maxBytes} bytes`);
			}
			chunks.push(value);
		}
	} finally {
...
export async function readLimitedJson<T>(request: Request, maxBytes: number): Promise<T> {
	...
	const bytes = await readLimitedStream(request.body!, maxBytes);
...
/**
 * Read up to `maxBytes` from a stream and return the raw bytes.
